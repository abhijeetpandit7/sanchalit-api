...
const getScheduledQuotes = async (req) => {
  ...
  const hasTomorrowsQuote = quoteCollection.scheduledList.some(({ forDate }) =>
    moment(forDate).isSame(tomorrow, "day")
  );
...
const mergeQuote = async (sourceUserId, destinationUserId) => {
  ...
  if (quoteCollectionOfSourceUser.customList.length) {
    await Quote.updateMany(
      {
        isCustom: true,
        userId: new mongoose.Types.ObjectId(String(sourceUserId)),
      },
      {
        $set: {
          userId: new mongoose.Types.ObjectId(String(destinationUserId)),
        },
      }
    );
  }
  if (!quoteCollectionOfDestinationUser) {
    const newUserQuoteCollection = new QuoteCollection({
